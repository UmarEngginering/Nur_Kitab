...
doa_title = lines[0].strip()
doa_body = lines[1].strip()

sentences = [s.strip() + '.' if not s.strip().endswith('.') else s.strip() for s in doa_body.split('. ') if s.strip()]
if sentences and sentences[-1] == 'اَلْفَاتِحَة.':
    sentences[-1] = 'اَلْفَاتِحَة'
...
for item in data['data']:
    if item['judul'] == 'Maulid Barzanji Nadzom':
        item['bacaan'].append(doa_title)
        item['bacaan'].extend(sentences)
        break
